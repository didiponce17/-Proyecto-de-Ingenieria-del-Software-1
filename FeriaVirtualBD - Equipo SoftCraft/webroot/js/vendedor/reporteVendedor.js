...
function generarReporte() {
  ...
  const tablavendedor = document.createElement("table");
  tablavendedor.className = "table";
  const encabezados = [
    "Fecha",
    "Cliente",
    "Categoría",
  ...
  const encabezadosRow = tablavendedor.insertRow();
  ...
  datosReporte.forEach((dato) => {
    const row = tablavendedor.insertRow();
    row.insertCell().textContent = dato.fecha;
    row.insertCell().textContent = dato.cliente;
    row.insertCell().textContent = dato.categoria;
    row.insertCell().textContent = dato.producto;

    //row.insertCell().textContent = dato.cantidad;
    // row.insertCell().textContent = (dato.precio * dato.cantidad).toFixed(2);
  ...
  const reporteDiv = document.getElementById("reporte-vendedor");
  reporteDiv.innerHTML = "";
  reporteDiv.appendChild(tablavendedor);
}
...
function descargarCSV() {
  ...
  csvContent +=
    "Fecha,Cliente,Categoría,Producto,Precio por Unidad (CRC),Cantidad Vendida (kg),Total Ventas (CRC)\r\n"; // Encabezados
  datosReporte.forEach((dato) => {
    const row = `${dato.fecha},${dato.cliente},${dato.categoria},${
      dato.producto
    },${dato.precio},${dato.cantidad},${(dato.precio * dato.cantidad).toFixed(
      2
    )}\r\n`;
    csvContent += row;
  });
